...
built-in tools (`read` `write` `edit` `bash` `grep` `find` `ls`) and
cooperating extension tools into compact, configurable blocks — a two-line
default plus optional one-line reasoning and result layouts — so the transcript
reads like a narrative, not a wall of boxes.
...
- **Line 1** — dedicated Nerd Font tool icon/name and the model's **goal/reasoning**; pending and failed calls add `󰔟` and `󰅖` status glyphs after the name.
- **Line 2** — the concrete target (path/command/pattern) and a colored result summary.

A [Nerd Font](https://www.nerdfonts.com/) is required for the tool and status glyphs.
...
/tidy background on
/tidy background off
/tidy background toggle
/tidy background status
```
...
Tool backgrounds are on by default and follow Pi's active theme: pending,
successful, and failed rows use the corresponding tool state color. The
`background` commands control only that paint; icons, text colors, summaries,
and layout stay unchanged.

A successful state, layout, or background change is saved to
...
reports when the override is active. A missing, unreadable, or malformed
enablement config defaults to enabled.
...
Mirrors a clean, theme-agnostic palette with one Nerd Font icon per tool:
...
Cooperating extension tools have distinct lifecycle-oriented icons:

| Tool               | Icon | Color   |
| ------------------ | ---- | ------- |
| `subagent_spawn`   | 󰚩    | magenta |
| `subagent_wait`    | 󰚭    | cyan    |
| `subagent_cancel`  | 󰜺    | red     |
| `subagent_check`   | 󰋼    | cyan    |
| `subagent_list`    | 󰉹    | magenta |
| `executor_execute` | 󰆍    | magenta |
| `executor_skills`  | 󰂺    | yellow  |
| `executor_resume`  | 󰐊    | green   |
| `webfetch`         | 󰖟    | cyan    |
| `websearch`        | 󰍉    | magenta |

Pending tools add a dim `󰔟` after the name, failures add a red `󰅖`, and successful
tools stay quiet with no redundant status mark.
...
- Tool blocks use Pi's one-column transcript padding and theme-native state backgrounds without a border
- Every line is truncated to the live terminal width (ANSI-aware) so nothing wraps
- Pi's native separator remains between independently rendered tool rows

Raw ANSI is intentional for the foreground palette; background paint is theme-native and can be disabled independently.

## Extension tools

Pi does not expose foreign tool executors through `getAllTools()`, so clean-tools
cannot safely replace arbitrary third-party renderers after registration. It does
support tools whose owning extension opts into the synchronous
`pi-tidy-tools:decorate-tool` event before calling `registerTool`.

The local integrations currently cover:

- `subagent_spawn`, `subagent_wait`, `subagent_cancel`, `subagent_check`, and
  `subagent_list`
- Executor MCP direct tools (`executor_execute`, `executor_skills`, and
  `executor_resume`)
- `webfetch` and `websearch`

These tools keep their original executor, argument preparation, metadata, and
results. Clean-tools adds the selected layout, optional reason-first schema, live
elapsed state, expansion, and tool-specific summaries. Uncooperative third-party
tools retain their own rendering.

Extension authors can opt in without depending on this package:

```ts
const request = { version: 1 as const, tool };
pi.events.emit("pi-tidy-tools:decorate-tool", request);
pi.registerTool(request.tool);
```

When clean-tools is disabled or absent, the request is unchanged.
...
shows permanent Nerd Font icons at normal and narrow widths: the scripts run the
