...
async function registerEnabledExtension(background = true): Promise<ExtensionHarness> {
  ...
  try {
    const enabledExtension = background
      ? hermeticExtension
      : createTidyExtension({ createIntegration: absentIntegration, loadBackground: () => false });
    await enabledExtension({
...
test("tool blocks support default reasoning and result layouts without completion ages", () => {
  ...
  assert.deepEqual(defaultBlock, [
    "󰏫 edit update the renderer",
    "  index.ts → +1/-1",
  ]);
  assert.deepEqual(plain("reasoning"), [
    "󰏫 edit update the renderer → +1/-1",
  ]);
  const resultBlock = plain("result");
  assert.deepEqual(resultBlock, ["󰏫 edit index.ts → +1/-1"]);
  assert.doesNotMatch(resultBlock[0], /\bago\b|update the renderer/);
});

test("tool blocks use dedicated Nerd Font icons and inline semantic status", () => {
  const cases = [
    ["read", "󰈙", { path: "a.ts", reasoning: "inspect source" }],
    ["write", "󰆓", { path: "a.ts", content: "done", reasoning: "write source" }],
    ["edit", "󰏫", { path: "a.ts", reasoning: "apply change" }],
    ["bash", "", { command: "npm test", reasoning: "run checks" }],
    ["grep", "󱎸", { pattern: "needle", reasoning: "find matches" }],
    ["find", "󰈞", { pattern: "*.ts", reasoning: "find files" }],
    ["ls", "󰝰", { path: "src", reasoning: "list source" }],
  ] as const;
  const result = {
    content: [{ type: "text", text: "done" }],
    details: { diff: "+new\n-old" },
  ...
  for (const [name, icon, args] of cases) {
    for (const mode of ["default", "reasoning", "result"] as const) {
      const success = withoutAnsi(buildToolBlock(name, args, result, { mode })[0]);
      const pending = withoutAnsi(buildToolBlock(name, args, result, { mode, isPartial: true })[0]);
      const failure = withoutAnsi(buildToolBlock(name, args, result, { mode, isError: true })[0]);
      assert.match(success, new RegExp(`^${icon} ${name}(?! [󰔟󰅖])`));
      assert.match(pending, new RegExp(`^${icon} ${name} 󰔟`));
      assert.match(failure, new RegExp(`^${icon} ${name} 󰅖`));
      for (const line of buildToolBlock(name, args, result, { mode })) {
        assert.ok(fitToolLine(line, 28).length > 0);
      }
    }
  }

  assert.deepEqual(
    buildTurnDiffBlock([{ tool: "edit", path: "a.ts", diff: "+new" }]).map(withoutAnsi),
    [" last turn diff (1 file)", "󰏫 a.ts", "+new"]
  );
});
...
test("status reports an active environment override without reloading", async () => {
  ...
  assert.match(notices[0], /off, mode default, background on \(PI_TIDY_TOOLS override\)/);
...
test("registered renderers summarize native result shapes for every owned tool", async () => {
  ...
  assert.deepEqual(
    ...
    [" 󰈙 read inspect source", "   a.ts → 2 lines"]
  );
  assert.deepEqual(
    render(
      "write",
  ...
  assert.deepEqual(
    ...
    [" 󰆓 write one.txt", "   one.txt → 1 line"]
  );
  assert.deepEqual(
    render(
      "edit",
    ...
    [" 󰏫 edit apply update", "   a.ts → applied"]
  );
  assert.deepEqual(
    render(
      "edit",
    ...
    [" 󰏫 edit 󰅖 a.ts", "   a.ts → replacement missing"]
  );
  assert.deepEqual(
    render(
      "bash",
  ...
  assert.deepEqual(
    ...
    [" 󱎸 grep none", "   none → 0 matches in 0 files"]
  );
  assert.deepEqual(
    render(
      "grep",
    ...
    [" 󱎸 grep one in src", "   one in src → 1 match in 1 file"]
  );
  assert.deepEqual(
    render("find", { pattern: "*.ts" }, { message: "a.ts\nb.ts" }),
    [" 󰈞 find *.ts", "   *.ts → 2 files"]
  );
  assert.deepEqual(
    render(
      "ls",
    ...
    [" 󰝰 ls 󰅖 missing", "   missing → not found"]
  );
  assert.deepEqual(new Set(backgrounds), new Set(["toolSuccessBg", "toolErrorBg"]));
});

test("registered renderers use state backgrounds and keep permanent icons", async () => {
  const { commands, events, messages, tools } = await registerEnabledExtension();
  const backgroundCalls: string[] = [];
  const backgroundAnsi = "\x1b[48;5;1m";
  const theme = {
    bg: (name: string, text: string) => {
      backgroundCalls.push(name);
      return `${backgroundAnsi}${text}\x1b[49m`;
    },
  };

  const success = tools.get("read").renderResult(
    { output: "one\ntwo" },
    {},
    theme,
    { args: { path: "a.ts", reasoning: "inspect source" }, isError: false },
  );
  const paintedSuccess = success.render(28)[0] as string;
  assert.match(withoutAnsi(paintedSuccess), /^ 󰈙 read /);
  assert.ok(paintedSuccess.split(backgroundAnsi).length > 2);

  const failure = tools.get("read").renderResult(
    { error: "not found" },
    {},
    theme,
    { args: { path: "a.ts", reasoning: "inspect source" }, isError: true },
  );
  assert.match(renderedLines(failure, 28)[0], /^ 󰈙 read 󰅖 /);

  const pending = tools.get("bash").renderCall(
    { command: "sleep 1", reasoning: "wait briefly" },
    theme,
    { isPartial: true, toolCallId: "live", invalidate() {} },
  );
  assert.match(renderedLines(pending, 28)[0], /^  bash 󰔟 /);
  ...
  await events.get("turn_end")!();
  await commands.get("diff").handler("", { ui: { notify() {} } });
  assert.deepEqual(messages[0].details.rows.map(withoutAnsi), [" last turn diff (1 file)", "󰏫 a.ts", "+new"]);
  assert.deepEqual(new Set(backgroundCalls), new Set(["toolPendingBg", "toolSuccessBg", "toolErrorBg"]));
});

test("background off leaves tool rows unpainted", async () => {
  const { tools } = await registerEnabledExtension(false);
  const backgrounds: string[] = [];
  const theme = {
    bg: (name: string, text: string) => {
      backgrounds.push(name);
  ...
  const component = tools.get("read").renderResult(
    { output: "one\ntwo" },
    {},
    theme,
    { args: { path: "a.ts", reasoning: "inspect source" }, isError: false },
  );
  assert.deepEqual(renderedLines(component), [" 󰈙 read inspect source", "   a.ts → 2 lines"]);
...
test("registered result renderers tolerate absent optional inputs", async () => {
  ...
  assert.deepEqual(new Set(backgrounds), new Set(["toolSuccessBg"]));
});
...
test("registered message renderer fits stored and restored recap rows", async () => {
  ...
  assert.deepEqual(storedLines, [" a very lon…", " l… → result"]);
...
test("result mode keeps native schemas and management commands preserve state", async () => {
  ...
  const script = `
		...
		await commands.get("tidy").handler("background toggle", context);
  ...
  try {
    ...
    assert.equal(observed.reloads, 3);
    assert.ok(
    ...
    assert.ok(observed.notices.some((notice: string) => /background set to off/.test(notice)));
    assert.ok(
...
test("buildToolBlock renders exact collapsed summaries across native shapes", () => {
  ...
  assert.deepEqual(
    ...
    ["󰈙 read inspect source", "  src/ file.ts → 2 lines"]
  );
  assert.deepEqual(
    plain(
      "grep",
  ...
  assert.deepEqual(
    ...
    ["󰈞 find find files", "  *.ts in src → 2 files"]
  );
  assert.deepEqual(
    plain(
      "ls",
    ...
    ["󰝰 ls list source", "  src → 2 entries"]
  );
  assert.deepEqual(
    plain(
      "edit",
...
test("registered APIs expose exact completions and reason-first tool metadata", async () => {
  ...
  assert.deepEqual(tidy.getArgumentCompletions(""), [
    ...
    { value: "background on", label: "background on" },
    { value: "background off", label: "background off" },
    { value: "background toggle", label: "background toggle" },
    { value: "background status", label: "background status" },
