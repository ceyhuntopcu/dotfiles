...
test("background defaults on and accepts only persisted booleans", async () => {
  await withTempConfig(async (configPath) => {
    assert.equal(loadTidyBackground(configPath), true);
    for (const [value, expected] of [
      ...
      await writeFile(configPath, JSON.stringify({ background: value }));
      assert.equal(loadTidyBackground(configPath), expected);
    }
...
test("saves preserve sibling settings while replacing owned values exactly", async () => {
  await withTempConfig(async (configPath) => {
    ...
    await saveTidyBackground(false, configPath);
    assert.equal(loadTidyMode(configPath), "result");
    assert.equal(loadTidyBackground(configPath), false);
    assert.equal(
      await readFile(configPath, "utf8"),
      '{\n  "sibling": {\n    "nested": [\n      1,\n      "two"\n    ]\n  },\n  "enabled": false,\n  "mode": "result",\n  "background": false\n}\n'
    );
