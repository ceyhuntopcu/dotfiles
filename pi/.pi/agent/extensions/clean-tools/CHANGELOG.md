...
- Tool rows use Pi's pending, success, and error theme backgrounds by default; `/tidy background on|off|toggle|status` controls the background independently of the row layout and icons.

### Changed

- Self-rendered tool rows now use Pi's one-column transcript padding so their icons align with surrounding text.
- Dedicated Nerd Font icons identify each tool, with inline `󰔟` pending and `󰅖` failure status.
- `/diff` uses dedicated Nerd Font icons for its heading and changed files.

### Removed

- Optional emoji visibility and `/tidy icons on|off|status`; Nerd Font icons are now permanent and require a Nerd Font-enabled terminal.
