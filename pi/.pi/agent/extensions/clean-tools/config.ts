...
/** Load whether tool rows use Pi's state background colors. */
export function loadTidyBackground(configPath = CONFIG_PATH): boolean {
	try {
		const background = JSON.parse(readFileSync(configPath, "utf8"))?.background;
		if (typeof background === "boolean") return background;
	} catch {
		// Missing and malformed config preserve visible backgrounds.
	}
	return true;
}
...
/** Persist tool row background visibility while preserving sibling settings. */
export async function saveTidyBackground(background: boolean, configPath = CONFIG_PATH): Promise<void> {
	await updateConfig({ background }, configPath);
}
