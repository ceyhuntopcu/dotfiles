...
export default function (pi: ExtensionAPI) {
	...
	// ── Render caches ──
	// Key: `${text.length}:${width}:${text.slice(0,120)}` → rendered markdown lines
	const mdCache = new Map<string, string[]>();
	// Full transcript cache: only rebuild when state actually changes
	let transcriptCache: { width: number; lines: string[]; version: number } | null = null;
	let transcriptVersion = 0;

	function invalidateTranscriptCache(): void {
		transcriptVersion++;
	...
	function mdCacheKey(text: string, width: number): string {
		// Fast key: length + width + first 120 chars is collision-resistant enough
		return `${text.length}:${width}:${text.slice(0, 120)}`;
	}

	function renderMarkdownLines(text: string, width: number): string[] {
		if (!text) return [];
		const key = mdCacheKey(text, width);
		const cached = mdCache.get(key);
		if (cached) return cached;

		try {
			const md = new Markdown(text, 0, 0, mdTheme);
			const result = md.render(width);
	...
	function getTranscriptLinesInner(width: number, theme: ExtensionContext["ui"]["theme"]): string[] {
		if (
			transcriptCache &&
			transcriptCache.width === width &&
			transcriptCache.version === transcriptVersion
		) {
			return transcriptCache.lines;
		}

		if (thread.length === 0 && !pendingQuestion && !pendingAnswer && !pendingError) {
			const lines = [theme.fg("dim", "No BTW messages yet. Type a question below.")];
			transcriptCache = { width, lines, version: transcriptVersion };
			return lines;
		}
		...
		for (const item of thread.slice(-6)) {
			...
			// Assistant message rendered as markdown (cached)
	...
	function scheduleOverlayRefresh(): void {
		...
		// Increased from 16ms → 50ms: still smooth but far fewer redundant renders
