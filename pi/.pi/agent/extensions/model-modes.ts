...
function getGlobalModesPath(): string {
    return path.join(getGlobalAgentDir(), "model-modes.json");
}

function getProjectModesPath(cwd: string): string {
    return path.join(cwd, ".pi", "model-modes.json");
}
...
function computeModesPatch(base: ModesFile, next: ModesFile, includeCurrentMode: boolean): ModesPatch | null {
    ...
    for (const k of keys) {
        ...
        const fields: (keyof ModeSpec)[] = ["provider", "modelId", "thinkingLevel"];
...
function orderedModeNames(modes: Record<string, ModeSpec>): string[] {
    ...
    // NOTE: "custom" is an overlay mode and must not be selectable/persisted. "default" is a
    // forced housekeeping entry (ensureDefaultModeEntries always creates one) rather than a
    // mode the user actually asked for, so it's excluded from cycling/listing the same way.
    return Object.keys(modes).filter((name) => name !== CUSTOM_MODE_NAME && name !== "default");
}
...
function inferModeFromSelection(ctx: ExtensionContext, pi: ExtensionAPI, data: ModesFile): string | null {
    ...
    // In that case, treat thinkingLevel differences in model-modes.json as non-distinguishing.
...
type ModeRuntime = {
     ...
     * which is *not* persisted and not selectable via /model-modes.
     */
    currentMode: string;
...
/**
 * The exact model we just asked pi to switch to via applyMode(), tracked by
 * identity rather than timing. The `applying` flag above only protects against
 * the model_select event firing *before* pi.setModel() resolves; if it fires
 * afterward (a real race under slow/async providers), `applying` has already
 * been reset and the handler would wrongly treat our own switch as a manual
 * one, flipping back to the "custom" overlay. Comparing against this instead
 * of relying purely on flag timing closes that race regardless of how late
 * the event arrives.
 */
let pendingAppliedTarget: { provider?: string; modelId?: string } | null = null;
...
async function applyMode(pi: ExtensionAPI, ctx: ExtensionContext, mode: string): Promise<void> {
    ...
    if (!spec) {
        if (ctx.hasUI) {
            ctx.ui.notify(`Unknown model mode: ${mode}`, "warning");
        }
        return;
    }
    ...
    try {
        ...
        if (spec.provider && spec.modelId) {
            ...
            if (m) {
                pendingAppliedTarget = { provider: spec.provider, modelId: spec.modelId };
...
const MODE_UI_CONFIGURE = "Configure model modes…";
const MODE_UI_ADD = "Add model mode…";
...
function validateModeNameOrError(
    ...
    if (!name) return "Model mode name cannot be empty";
    if (/\s/.test(name)) return "Model mode name cannot contain whitespace";
    if (isReservedModeName(name)) return `Model mode name \"${name}\" is reserved`;
...
async function selectModeUI(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
    ...
    while (true) {
        ...
        const choice = await ctx.ui.select(`Model mode (current: ${runtime.currentMode})`, [...names, MODE_UI_CONFIGURE]);
        if (!choice) return;
...
async function configureModesUI(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
    ...
    while (true) {
        await ensureRuntime(pi, ctx);
        const names = orderedModeNames(runtime.data.modes);
        const choice = await ctx.ui.select("Configure model modes", [...names, MODE_UI_ADD, MODE_UI_BACK]);
...
async function addModeUI(pi: ExtensionAPI, ctx: ExtensionContext): Promise<string | undefined> {
    ...
    while (true) {
        const raw = await ctx.ui.input("New model mode name", "e.g. smart, rush, deep");
        ...
        await persistRuntime(pi, ctx);
        ctx.ui.notify(`Added model mode \"${name}\"`, "info");
        return name;
    }
}
...
async function editModeUI(pi: ExtensionAPI, ctx: ExtensionContext, mode: string): Promise<void> {
    ...
    while (true) {
        ...
        const action = await ctx.ui.select(
            `Edit model mode \"${modeName}\"  model: ${modelLabel}  thinking: ${thinkingLabel}`,
            actions,
        );
        ...
        if (action === "Delete mode") {
            ...
            ctx.ui.notify(`Deleted model mode \"${modeName}\"`, "info");
            return;
        }
    }
}
...
async function renameModeUI(pi: ExtensionAPI, ctx: ExtensionContext, oldName: string): Promise<string | undefined> {
    ...
    if (isDefaultModeName(oldName)) {
        ctx.ui.notify(`Cannot rename default model mode \"${oldName}\"`, "warning");
        return oldName;
    }
...
export default function (pi: ExtensionAPI) {
    pi.registerCommand("model-modes", {
        description: "Select model mode",
        handler: async (args, ctx) => {
            ...
            if (tokens[0] === "store") {
                ...
                if (!target) {
                    if (!ctx.hasUI) return;
                    const names = orderedModeNames(runtime.data.modes);
                    target = await ctx.ui.select("Store current selection into model mode", names);
                    if (!target) return;
                }
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        // If a startupMode is configured, force it except in print mode. Print mode
        // includes headless subagents, where an explicitly requested model must win.
        const startupMode = runtime.data.startupMode;
        if (
            ctx.mode !== "print" &&
            startupMode &&
            startupMode !== CUSTOM_MODE_NAME &&
            runtime.data.modes[startupMode]
        ) {
    ...
    pi.on("model_select", async (event, ctx) => {
        ...
        // while we are in the middle of applying a mode. `applying` catches the common case where
        // this event fires before pi.setModel() resolves; comparing against pendingAppliedTarget
        // catches the race where it fires after — identity-based, so it doesn't depend on timing.
        const matchesPendingApply =
            pendingAppliedTarget !== null &&
            event.model.provider === pendingAppliedTarget.provider &&
            event.model.id === pendingAppliedTarget.modelId;
        if (runtime.applying || matchesPendingApply) {
            pendingAppliedTarget = null;
            return;
        }
